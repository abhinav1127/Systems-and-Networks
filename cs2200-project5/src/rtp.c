...
static packet_t *packetize(char *buffer, int length, int *count) {
    ...
    *count = length / MAX_PAYLOAD_LENGTH;
    ...
    for (int i = 0; i < *count - 1; i++) {
        ...
        memcpy(packets[i].payload, buffer + i * MAX_PAYLOAD_LENGTH, (size_t) MAX_PAYLOAD_LENGTH);
    ...
    if (length % MAX_PAYLOAD_LENGTH != 0) {
        ...
        packets[*count - 1].payload_length = length % MAX_PAYLOAD_LENGTH;

    } else {
        packets[*count - 1].payload_length = MAX_PAYLOAD_LENGTH;
    }

    packets[*count - 1].type = LAST_DATA;
            	
    memcpy(packets[*count - 1].payload, buffer + (*count - 1) * MAX_PAYLOAD_LENGTH, (size_t) MAX_PAYLOAD_LENGTH);

    packets[*count - 1].checksum = checksum(packets[*count - 1].payload, packets[*count - 1].payload_length);
...
static void *rtp_recv_thread(void *void_ptr) {
    ...
    do {
           ...
           forceRepeat = 0;
            ...
            if (packet.type == DATA || packet.type == LAST_DATA) {
                ...
                if (packet.checksum == checksum(packet.payload, packet.payload_length)) {
                    ...
                    buffer = (char *) realloc(buffer, (size_t) buffer_length + (size_t) packet.payload_length);
        ...
        } while (packet.type != LAST_DATA || forceRepeat);
...
static rtp_connection_t *rtp_init_connection(int net_connection_handle) {
    ...
    rtp_connection->acknowledge = 0;
    rtp_connection->acksent = 0;
