...
void system_init(void) {
    ...
    frame_table = (fte_t*) (mem + (PAGE_SIZE * 0));
...
void proc_init(pcb_t *proc) {
    ...
    uint8_t* zeroOut = (mem + (freed * PAGE_SIZE));
    memset(zeroOut, 0, PAGE_SIZE);

    // pte_t* pageTableEntry = (pte_t*) zeroOut;
    // pageTableEntry -> pfn = freed;
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    // pte_t* pageTableBeginning = (pte_t*) mem[(paddr_t) PTBR << OFFSET_LEN];
    ...
    // if (pageEntry -> valid) {
    if (!pageTableBeginning[vpn].valid) {
        page_fault(address);
    }
    ...
    */
    ...
    pfn_t pfn = pageTableBeginning[vpn].pfn;

    //For clocksweep
    if (replacement == CLOCKSWEEP) {
        frame_table[pfn].timestamp = 1;
    }

    paddr_t physical_address = ((paddr_t) (pfn << OFFSET_LEN)) + (paddr_t) offset;
    ...
    if (rw == 'r') {
        ...
        pageTableBeginning[vpn].dirty = 1;
        stats.writes += 1;
    }
...
void proc_cleanup(pcb_t *proc) {
    ...
    pfn_t ptbrTemp = proc -> saved_ptbr;
    pte_t* pageTableBeginning = (pte_t*) (mem + (ptbrTemp * PAGE_SIZE));

    /* Iterate the page table and clean up each valid page */
    for (size_t i = 0; i < NUM_PAGES; i++) {
        pfn_t pfn = (pageTableBeginning + i) -> pfn;
        fte_t* frameEntry = &frame_table[pfn];

        if ((pageTableBeginning + i) -> valid) {
            frameEntry -> mapped = 0;
        }

        if ((pageTableBeginning + i) -> swap) {
            swap_free(&pageTableBeginning[i]);
            //(pageTableBeginning + i) -> swap = 0;
        }
    }

    /* Free the page table itself in the frame table */

    fte_t* frame_info = &frame_table[ptbrTemp];
    frame_info -> protected = 0;
    //For clocksweep
    if (replacement == CLOCKSWEEP) {
