...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        ...
        pfn_t correctPTBR = frame_table[victim_pfn].process -> saved_ptbr;
        pte_t* pageTableBeginning = (pte_t*) (mem + (correctPTBR * PAGE_SIZE));        
        // pte_t* pageTableBeginning = (pte_t*) mem + (PTBR * PAGE_SIZE);
        // pte_t* pageEntry = pageTableBeginning + vpn;

        void* desiredFrame = (void*) (mem + (PAGE_SIZE * victim_pfn));
        if (pageTableBeginning[vpn].dirty) {
            // swap_write(pageEntry, &victim_pfn);
            stats.writebacks += 1;
            swap_write(&pageTableBeginning[vpn], desiredFrame);
            
            // idk about this line:
            pageTableBeginning[vpn].dirty = 0;
        }

        pageTableBeginning[vpn].valid = 0;
        frame_table[victim_pfn].mapped = 0;

        //For clocksweep
        if (replacement == CLOCKSWEEP) {
            frame_table[victim_pfn].timestamp = 0;
        }
    }
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        timestamp_t currMin = get_current_timestamp();
        pfn_t minIndex = 0;

        for (int i = 0; i <NUM_FRAMES; i++) {
            if ((frame_table[i].timestamp < currMin) && !frame_table[i].protected) {
                currMin = frame_table[i].timestamp;
                minIndex = i;
            }
        }
        ...
        while (i) {
            ...
            if (!frame_table[clock].protected) {
                if (frame_table[clock].timestamp) {
                    frame_table[clock].timestamp = 0;
                } else {
                    clock += 1;
                    return clock - 1;
                }
